...
import BluetoothSupportDialog, { useBluetoothDialog } from './BluetoothSupportDialog';
import { 
  Bluetooth, 
  Power, 
  PlayCircle, 
...
const NavBar = ({
  ...
  const handleConnectClick = (type) => {
    if (!environment.hasWebBluetooth || (environment.isIOS && !environment.isBluefy)) {
      showDialog();
      return;
    }
        ...
        <div className="px-3 py-2 sm:px-6 sm:py-3 bg-gray-800/50">
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => isBikeConnected ? handleDisconnect() : handleConnectClick('bike')}
              ...
              <button
                onClick={() => isHeartRateConnected ? handleDisconnect() : handleConnectClick('hr')}
                className={`p-1.5 sm:px-3 sm:py-1.5 rounded-md flex items-center space-x-1 transition-all ${
                  isHeartRateConnected
                    ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20'
                    : 'bg-blue-500/10 text-blue-500 hover:bg-blue-500/20'
